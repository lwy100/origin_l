...
当前主页定位是匿名的旅行与生活兴趣展示页，品牌名为 `Signal Garden`。页面不展示真实姓名，不展示工作、公司或技术背景，公开联系邮箱为 `1142516819@qq.com`。核心内容包括四套视觉主题、旅行地图、随机推荐、轻松任务卡片和联系卡片。
...
除非明确要求，否则保持纯 HTML、CSS、JavaScript 实现，不引入打包工具或前端框架。HTML/CSS/JS 使用两个空格缩进。优先使用语义化 HTML、清晰的 class 命名，以及 CSS 变量维护颜色和通用样式。界面文案默认使用中文；如果已有双语设计，可保留英文作为辅助。保持年轻、自然、活泼且有审美的视觉方向，避免工作化、简历化或技术宣传语境。
...
当前没有自动化测试框架。提交前应启动本地服务器并在浏览器中检查页面。重点验证：移动端菜单、主题切换、出门签、旅行地图、随机推荐、轻松任务卡片、复制邮箱、响应式布局和 GitHub Pages 路径兼容性。如果新增较多 JavaScript，请拆分为 `assets/js/modules/` 下的具名模块，并在 PR 或提交说明中写明手动测试内容。
...
不要重新引入根目录 `styles.css` 或 `script.js`；它们已迁移到 `assets/css/` 和 `assets/js/`。修改架构时同步更新 `README.md` 与 `docs/ARCHITECTURE.md`。涉及个人信息时，继续保持匿名展示，不要加入真实姓名、工作经历、公司信息或技术背景。
